...
contract MerchantEscrow is Ownable {
	...
	function payEscrow(
		uint256 _escrowId
	) external checkIfEscrowExists(_escrowId) {
	...
	function requestRefund(
		...
		require(
			escrow.escrowStatus == Status.PAID,
			"Escrow state has to be set to paid to request refund"
		);
	...
	function reviewRefund(
		uint256 _refundId,
		bool _refundDecision
	) external onlyOwner {
		Request storage request = activeRefundRequests[_refundId];
		Escrow storage escrow = escrows[request.escrowId];
		require(
			_refundId < activeRefundRequests.length,
			"Can't review nonexisting refund request"
		);
		require(
			escrow.escrowStatus == Status.REFUND_REQUESTED,
			"Payment can only be reversed if refund was requested"
		);
		...
		activeRefundRequests[_refundId] = activeRefundRequests[
	...
	modifier checkIfEscrowExists(uint256 _escrowId) {
		require(
			...
			"Escrow with provided escrowId does not exist"
		);
		_;
	}
	// =========================== Events ==============================
	...
	event RefundRequest(
		...
		address indexed seller,
		uint256 amount,
		uint256 activeRefundRequestCount,
		string refundReason
	);
